"""
Author       : Drew Heasman
Date updated : 30 November 2020
Description  : A function that takes a given point (in lat and long) and a 
               date, and outputs the current day location (in lat and long) of that point.

Notes        :         
"""
### Import libraries ###
import sys
sys.path.insert(1,r'C:\pygplates_python37')
import pygplates
###############################################################################################################
# Functions
def getPresentDayPoint(feature_file, rotation_file, latitude, longitude, reconstruction_time):
    """ Function takes a valid latitude and longitude from a specified time in the past, and returns the present day latitude and longitude of that point.

        
        Parameters:
           feature_file (FeatureCollection) : pyGplates feature collection needed to track point
           rotation_file (FeatureCollection): pyGplates feature collection needed to track point
           latitude (float)                 : past latitude
           longitude (float)                : past longitude
           reconstruction_time(int)         : past time
        
        Raises:
           ValueError: Latitude and Longitude must be within a valid feature at reconstruction time
        
        Returns:
            tuple: lat and long labelled tuple
    ...
    pygplates.reconstruct(features, rotation_model, reconstructed_features,
                              reconstruction_time, group_with_feature=True, anchor_plate_id=0)
    ...
    for feature, feature_reconstructed_geometries in reconstructed_features:
        ...
        for feature_reconstructed_geometry in feature_reconstructed_geometries:
            ...
            min_distance_to_feature = pygplates.GeometryOnSphere.distance(point,
    ...
    if min_distance_to_all_features != 0:
        raise ValueError("Latitude and Longitude must be within a valid feature at reconstruction time")
    else: 
        ...
        geometry = tracked_point.get_geometry(lambda property: True, pygplates.PropertyReturn.all)
